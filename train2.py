...
def train_model(model, train_dataset, valid_dataset, validation, train_loader, optimizer, epochs=1, device='cuda'):
...
def save_checkpoint(model, arch='vgg16', hidden_units=4096):
    checkpoint = {
        'arch': arch,
...
def main():
    ...
    model = train_model(model, train_dataset, valid_dataset, validation, train_loader, optimizer, criterion)
    
    #test the model
    
    #save the model
    checkpoint = save_checkpoint(model, args.arch, args.hidden_units)
